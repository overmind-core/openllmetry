...
def _store_prompt_attributes(span: Span, content: PromptString | str) -> None:
    if isinstance(content, PromptString):
        span.set_attribute(attrs.GEN_AI_PROMPT_ID, content.id)
        span.set_attribute(attrs.GEN_AI_PROMPT_HASH, content.hash)
        span.set_attribute(attrs.GEN_AI_PROMPT_KWARGS, json.dumps(content.kwargs))
        span.set_attribute(attrs.GEN_AI_PROMPT_TEMPLATE, content.template)
...
def request_processor(span: Span, kwargs: dict, type: str = "openai.chat") -> None:
    ...
    if type == "openai.chat":
        ...
        # ponytail: last PromptString wins when multiple are found; ceiling is
        # MAX_PROMPT_COUNT. Upgrade: collect all and emit multiple span events.
        for message in kwargs.get("messages", []):
            content = message.get("content")
            _walk_and_store_prompts(span, content, prompt_count)

    elif type == "openai.responses":
