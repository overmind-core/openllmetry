...
def test_openai_chat_with_promptstring_and_blocks():
    ...
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == prompt2.hash
    assert json.loads(
        span.attributes[attrs.GEN_AI_PROMPT_KWARGS]
    ) == prompt2.kwargs
...
def test_openai_responses_instructions_and_input():
    ...
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == ipt.hash
    assert json.loads(
        span.attributes[attrs.GEN_AI_PROMPT_KWARGS]
    ) == ipt.kwargs
...
def test_anthropic_messages_system_and_messages():
    ...
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == message_prompt.hash
    assert json.loads(
        span.attributes[attrs.GEN_AI_PROMPT_KWARGS]
...
def test_google_genai_responses_contents_and_system_instruction():
    ...
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == content_prompt.hash
    assert json.loads(
        span.attributes[attrs.GEN_AI_PROMPT_KWARGS]
...
def test_agno_agent_run_prompt():
    ...
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == prompt.hash
    assert json.loads(
        span.attributes[attrs.GEN_AI_PROMPT_KWARGS]
    ) == prompt.kwargs


def test_multiple_prompts_last_wins():
    """With the ValueError guard removed, multiple PromptStrings result in the last one winning."""
    span = _make_span()

    prompts = [
        PromptString(id=str(i), template="T {i}", kwargs={"i": i}) for i in range(3)
    ]
    ...
    # last prompt wins
    assert span.attributes[attrs.GEN_AI_PROMPT_HASH] == prompts[-1].hash
